...
async def get_latest_scan(
    ...
    """Return results of the most recent screener scan."""
    # Get the latest scanned_at timestamp
    result = await db.execute(
        select(func.max(ScreenerSnapshot.scanned_at))
    )
    ...
    return {
        ...
        "candidates": [
            {
                "symbol": s.symbol,
                "avg_candle_size_pct": s.avg_candle_size_pct,
                "atr": s.atr,
                ...
                "price_range_pct": s.price_range_pct,
...
async def trigger_scan(
    ...
    """Manually trigger a screener scan."""
