...
def _elapsed_hours(entry_time: datetime) -> float:
    ...
    return round((datetime.now(timezone.utc) - entry_time).total_seconds() / 3600, 2)
...
class UpdateTrendSignalsUseCase:
    ...
    async def execute(self, db: AsyncSession) -> None:
        result = await db.execute(select(TrendSignalLog).where(TrendSignalLog.status == "OPEN"))
        ...
        for row in open_signals:
            try:
                ...
                if candles_1h and len(candles_1h) >= 22:
                    ...
                    ema_s = ema21_series(closes)
                    if ema_s:
                        ema_now = ema_s[-1]
                        ema_exit = (
                            (row.side == "LONG" and closes[-1] < ema_now) or
                            (row.side == "SHORT" and closes[-1] > ema_now)
                        )
                        if ema_exit:
                            row.exit_price = current
                            row.exit_time = datetime.now(timezone.utc)
