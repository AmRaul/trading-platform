            ...
            async with semaphore:
                try:
                    ...
                    if existing.scalar_one_or_none():
                        return
                    db.add(TrendSignalLog(
                        symbol=signal.symbol, side=signal.side,
                        entry_price=signal.entry_price, ema21_4h=signal.ema21_4h,
                        ema21_1h=signal.ema21_1h, trend_4h=signal.trend_4h,
                        pullback_1h=signal.pullback_1h, trigger_15m=signal.trigger_15m,
                        stop_price=signal.stop_price, stop_pct=signal.stop_pct, status="OPEN",
                    ))
                    logged += 1
                    logger.info(f"[TREND] {signal.side} {symbol} @ {signal.entry_price}")
